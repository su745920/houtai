    ...
    <div class="layui-item">
        ...
        <div class="layui-inline" style="margin-left: 10px;">
            ...
            <div class="layui-input-inline" style="width: 90px">
                <select name="from_name" id="from_name" class="layui-input">
        ...
        <div class="layui-inline" style="margin-left: 10px;">
            <label>产出币种</label>
            <div class="layui-input-inline" style="width: 90px">
                <select name="to_name" id="to_name" class="layui-input">
                    <option value="">所有</option>
                    @foreach ($currencies as $key => $currency)
                    <option value="{{ $currency->id }}">{{ $currency->name }}</option>
                    @endforeach
        ...
        layui.use(['element', 'form', 'layer', 'table', 'laydate'], function() {
            ...
            var data_table =   table.render({
                ...
                cols: [[
                    ...
                    ,{field: 'user_id', title: '会员账号', width: 170,templet: '<div>@{{d.user.account_number}}</div>'}
                    ,{field: 'from_name', title: '锁定币种', width: 110, sort: true}
                    ,{field: 'to_name', title: '产出币种', width: 110, sort: true}
                    ,{field: 'money', title: '金额', width: 80,  sort: true}
                    ,{field: 'day', title: '天数', width: 80, sort: true, hide: false}
                    ,{field: 'interest', title: '利息', width: 120, sort: true, hide: false,default:0}
                    ,{field: 'status', title: '交易状态', width: 100, sort: true}
                    ,{field: 'created_at', title: '下单时间', width: 170, sort: true}
                    ,{field: 'complete_at', title: '完成时间', width: 170, sort: true, hide: false}
            ...
            table.on('tool(order_list)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                var tr = obj.tr;
                if (layEvent === 'edit') { //编辑
                    layer_show('编辑交易', '/agent/micro_order_edit?id=' + data.id);
                }
            });
