...
<script type="text/html" id="addsonTpl">
  @{{#  if(d.status == 0){ }}
  <i class="layui-icon layui-icon-rate"style="font-size: 16px; color: #2A64FB;">挂单中</i>
  @{{#  } else if(d.status == 1) { }}
  <i class="layui-icon layui-icon-rate-half"style="font-size: 16px; color: #2A64FB;">交易中</i>
  @{{#  } else if(d.status == 2) { }}

  <i class="layui-icon layui-icon-refresh-3"style="font-size: 16px; color: red;">平仓中</i>
  @{{#  } else if(d.status == 3) { }}

  <i class="layui-icon layui-icon-rate-solid"style="font-size: 16px; color: #2A64FB;">已平仓</i>
  @{{#  } else if(d.status == 4) { }}

  <i class="layui-icon layui-icon-close-fill"style="font-size: 16px; color: #F56C6C;">已撤单</i>
  @{{#  } }}
</script>
    ...
    layui.use(['index','element', 'form','table', 'layer', 'laydate'], function () {
        ...
        table.render({
            ...
            , cols: [[
                ...
                , { field: 'user_name', title: '用户名', minWidth: 150, event: "getsons", style: "color: #fff;background-color: #67C23A;" }
