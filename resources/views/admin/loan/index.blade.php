     ...
     function debounce(func, wait) {
        ...
        layui.use(['table','form'], function(){
            ...
            var data_table = table.render({
                elem: '#loanOrderList'
                ,url: '{{url('admin/loan_order_list')}}' //数据接口
                ,page: true //开启分页
                ,id:'mobileSearch'
                ,toolbar: true
                ,height: 'full-60'
            ...
            table.on('tool(test)', function(obj) {
                var data = obj.data;
                if (obj.event === 'reset') { //通过
                    layer.confirm('确定要重置订单吗？', debounce(function (index) {
                        //向服务端发送删除指令
                        $.ajax({
                            url: "{{url('admin/loan_order_reset')}}",
                            type: 'post',
                            dataType: 'json',
                            data: {id: data.id},
