...
<script type="text/html" id="barDemo">
    @if(strpos($authorityList,"2801")>0)
...
<script type="text/html" id="switchTpl">
    @if(strpos($authorityList,"2702")>0)
    <input type="checkbox" name="status" value="@{{d.id}}" lay-skin="switch" lay-text="是|否" lay-filter="status" @{{ d.status == 1 ? 'checked' : '' }}>
    @else
        <input type="checkbox" name="status" value="@{{d.id}}" lay-skin="switch"  disabled
               lay-text="是|否" lay-filter="status" @{{ d.status == 1 ? 'checked' : '' }}>
    @endif
</script>
        ...
        layui.use(['element', 'form', 'layer', 'table', 'laydate'], function() {
            ...
            var data_table =   table.render({
                ...
                cols: [[
                    ...
                    ,{field: 'seconds', title: '时间(秒)', width: 130, sort: true, totalRowText: '小计'}
                    ,{field: 'profit_ratio', title: '收益率', width: 130}
                    ,{field: 'loss_ratio', title: '亏损率(%)', width: 130}
                    ,{field: 'min_amount', title: '最低起投数量', width: 130}
                    ,{field: 'max_amount', title: '最高起投数量', width: 130}
                    ,{field: 'status', title: '状态', width: 90, sort: true, templet: '#switchTpl', hide: false}
            ...
            table.on('tool(order_list)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                var tr = obj.tr;
                if (layEvent === 'edit') { //编辑
                    layer_show('编辑交易', '/admin/micro_config_edit?id=' + data.id);
                }
            });
            ...
            form.on('switch(status)', function(obj){
                ...
                $.ajax({
                    url:'{{url('admin/micro_config_status')}}',
                    type:'post',
                    dataType:'json',
                    data:{id:id},
