...
@section('scripts')
    ...
    <script type="text/html" id="operateBar">
        @{{# if (d.status == 1) { }}
            ...
            <!--<a class="layui-btn layui-btn-xs layui-btn-warm" lay-event="caution">风险设置</a>-->
        @{{# } }}
    </script>
    <script>
        var interval;
            ...
            layui.use(['element', 'form', 'layer', 'table', 'laydate'], function () {
                ...
                var lever_list = table.render({
                    ...
                    ,cols: [[
                        ...
                        ,{field: 'fact_profits', title: '当前盈亏', sort: true, width: 160, style:"background-color: #eaeaea;", totalRow: true}
                        ,{field: 'stop_loss_price', title: '止损价格', width: 140, totalRow: true}
                        ,{field: 'target_profit_price', title: '止盈价格', width: 140, totalRow: true}
                        ,{field: 'origin_caution_money', title: '初始保证金', width: 140, hide: true}
                        ,{field: 'caution_money', title: '可用保证金', width: 140}
                        ,{field: 'share', title: '手数', sort: true, width: 120}
                        ...
                        //,{field: 'caution_time', title: '风险时间', width: 140, totalRow: true}
                        //,{field: 'caution_price', title: '风险价格', width: 140, totalRow: true}
                        ,{fixed: 'right', title: '操作', width: 140, align: 'center', toolbar: '#operateBar'}
                    ]]
                });
                ...
                table.on('tool(leverList)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                    var data = obj.data
                        ,layEvent = obj.event
                        ,tr = obj.tr;
                    if(data.caution_time=='null'||data.caution_time==null) data.caution_time='';
                    if (layEvent === 'close') { //删除
                    layer.prompt({
                        title: '真的要强制平仓吗？请输入平仓价格',
                    ...
                    else if (layEvent === 'caution') { //风险设置
                        var index = layer.open({
                            title:'风险设置'
                            ,type:2
                            ,content: '/admin/Leverdeals/caution?id='+data.id+'&symbol='+data.symbol+'&account_number='+data.account_number+'&caution_time='+data.caution_time+'&caution_price='+data.caution_price
                            ,area: ['800px', '600px']
                            ,maxmin: true
                            ,anim: 3
                        });
