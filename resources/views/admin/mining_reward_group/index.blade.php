...
<script>
    ...
    layui.use(['table','form','laydate'], function(){
        ...
        table.render({
            elem: '#demo'
            ,url: "{{url('admin/miningRewardGroup/page')}}" //数据接口
            ,page: true //开启分页
            ,id:'mobileSearch'
            ,cols: [[ //表头
                {field: 'id', title: 'ID', width:80, sort: true}
                ,{field: 'base_money', title: '计息本金', width:180}
                ,{field: 'reward_username', title: '奖励用户', width:180}
                ,{field: 'create_date', title: '奖励时间', width: 180}

                ,{field: 'rate', title: '利率', width: 120}
                ,{field: 'days', title: '计息天数', width:140}

                ,{field: 'status', title: '状态', width: 120, templet: '#statustml'}
                ,{field: 'audit_date', title: '审核时间', width:180}
                ,{field: 'audit_username', title: '审核人', width:180}
        ...
        table.on('tool(test)', function(obj){
            ...
            if(obj.event === 'auditSuccessFn'){
                layer.confirm('您确认要审核通过吗', function(index){
                    $.ajax({
                        url:'{{url('admin/miningRewardGroup/tg')}}',
                        type:'post',
                        dataType:'json',
                        data:{id:data.id},
                ...
                layer.confirm('您确认不通过审核吗', function(index){
                    $.ajax({
                        url:'{{url('admin/miningRewardGroup/reject')}}',
                        type:'post',
                        dataType:'json',
                        data:{id:data.id},
                        success:function (res) {
