	...
	<form class="layui-form" method="POST">
		...
		<div class="layui-form-item">
			...
			<div class="layui-input-inline">
				<select name="user_id" class="" lay-filter="user_id" lay-search>
				    <option value="">全部</option>
					@foreach ($userList as $user)
			        <option value="@if (isset($user['id'])){{ $user['id'] }}@endif" @if(isset($news) && $news['user_id'] == $user['id']) selected @endif>@if (isset($user['id'])){{ $user['id'] }}@endif</option>
					@endforeach
			    </select>
			    <p style="color: #f66f6a;">仅针对站内信有用</p>
			</div>
		...
		<div class="layui-form-item">
			<label class="layui-form-label">缩略图Link</label>
			<div class="layui-input-block">
				<input type="text" placeholder="请输入缩略图Link"  name="thumbnail0"  class="layui-input" id="thumbnail0"  value="@if (isset($news['thumbnail'])){{$news['thumbnail']}}@endif">
			</div>
		</div>
		
		<div class="layui-form-item">
			<label class="layui-form-label">封面Link</label>
			<div class="layui-input-block">
				<input type="text" placeholder="请输入封面Link" name="cover0" class="layui-input" id="cover0" value="@if (isset($news['cover'])){{$news['cover']}}@endif">
			</div>
		</div>
		...
		<div class="layui-form-item">
			...
			<div class="layui-input-block">
				<textarea placeholder="请输入内容摘要" class="layui-textarea" name="abstract" style="height:50px;">@if (isset($news['keyword'])){{$news['keyword']}}@endif</textarea>
			</div>
		</div>
		...
		<div class="layui-form-item">
			...
			<div class="layui-input-block">
				<script id="news_content" name="content" type="text/plain" style="width:100%; height:200px;">@if (isset($news['content'])){!! $news['content'] !!}@endif</script>
			</div>
		</div>
		<div class="layui-form-item">
			<div class="layui-input-block">
