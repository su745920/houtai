...
<div class="layui-form">
     <button class="layui-btn layui-btn-primary" id="newsAdd">添加盲盒</button>
        ...
        layui.use(['element', 'form', 'layer', 'table', 'laydate'], function() {
            ...
            var data_table =   table.render({
                ...
                cols: [[
                    ...
                    ,{field: 'currency_id', title: '获得币种', width: 130}
                    ,{field: 'chance', title: '权重', width: 130, sort: true, totalRowText: '小计'}
            ...
            table.on('tool(order_list)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                var tr = obj.tr;
                if (layEvent === 'edit') { //编辑
                    layer_show('编辑盲盒', '/admin/blind_config_edit?id=' + data.id);
                }else if(layEvent === 'del'){
                     layer.confirm('真的删除行么?可能会导致系统崩溃！', function(index){
                         $.ajax({
                            url:'<?php echo e(url('admin/blind_del')); ?>',
                            type:'get',
                            dataType:'json',
                            data:{id:data.id},
            ...
            form.on('switch(status)', function(obj){
                ...
                $.ajax({
                    url:'{{url('admin/blind_config_status')}}',
                    type:'post',
                    dataType:'json',
                    data:{id:id},
                    success:function (res) {
            ...
            $('#newsAdd').click(function() {
                var index = layer.open({
                    title:'添加盲盒'
                    ,type:2
                    ,content: '/admin/blind_config_add'
                    ,area: ['800px', '600px']
                    ,maxmin: true
                    ,anim: 3
                });
