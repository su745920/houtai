...
@section('page-content')
    <div style="margin-top: 10px;width: 100%;margin-left: 10px;">
        @if(strpos($authorityList,"1601")>0)
        <button class="layui-btn layui-btn-normal layui-btn-radius" onclick="layer_show('添加机器人','{{url('admin/robot/robot_add')}}')">添加机器人</button>
        @endif
    ...
    <script type="text/html" id="barDemo">
        @if(strpos($authorityList,"1602")>0)
        <a class="layui-btn layui-btn-xs" lay-event="edit">编辑</a>
        @endif

        @if(strpos($authorityList,"1603")>0)
...
@section('scripts')
    <script>
        ...
        layui.use(['table','form'], function(){
            ...
            table.render({
                elem: '#demo'
                ,url: '{{url('admin/robot/robot_list')}}' //数据接口
                ,page: true //开启分页
                ,id:'mobileSearch'
                ,cols: [[ //表头
                    {field: 'id', title: 'ID', Width:30, sort: true}
                    ...
                    ,{field: 'min_need_second', title: '最小频率(秒)', minWidth:80}
                    ,{field: 'max_need_second', title: '最大频率(秒)', minWidth:80}
                    ...
                    // ,{field: 'is_lever', title: '是否合约币', minWidth:80, templet: '#level'}
            ...
            form.on('switch(sexDemo)', function(obj){
                var id = this.value;
                $.ajax({
                    url:'{{url('admin/robot/newauto_start')}}',
                    type:'post',
                    dataType:'json',
                    data:{id:id},
            ...
            table.on('tool(test)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('真的删除行么', function(index){
                        $.ajax({
                            url:'{{url('admin/robot/auto_robot_del')}}',
                            type:'post',
                            dataType:'json',
                            data:{id:data.id},
                            success:function (res) {
                    ...
                    layer_show('编辑机器人','{{url('admin/robot/robot_add')}}?id='+data.id);
                }
            });
