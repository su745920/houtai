    ...
    */
    ...
    public function doLockmining(){
        ...
        foreach ($list as $val) {
            ...
            $createdDatetime = Carbon::createFromTimestamp($val->created_at);
            // 判断今天是否更新过
            ...
            if(!$isToday) {
                ...
                try{
                    ...
                    if($created_at+$day <= time()){
                        ...
                        // 返本金
                        $this->setLog(Carbon::now()->toDateTimeString()."开始锁仓挖矿返本金===>".$val->id);
                        ...
                        $this->change_wallet_balance($wallet, 4, -$number, AccountLog::RELEASE_LOCK_MINING, '释放锁仓挖矿减少冻结资产','Release lock up mining to reduce frozen assets',true);
                        ...
                        if(empty($val->rate_max)) {
                            continue;
                        }
                        if(bcmul($val->rate_max,100) <= bcmul($val->rate,100)) {
                            continue;
                        }
                        $randomNumber = mt_rand(bcmul($val->rate,100), bcmul($val->rate_max,100));
                        $rate = bcdiv($randomNumber,100,2);
                        // 判断存入时间是否大于一天
                        $isGreaterThanOneDay = $createdDatetime->diffInDays(Carbon::now()) >= 1;
                        if($isGreaterThanOneDay) {
                            $this->setLog(Carbon::now()->toDateTimeString()."开始结算锁仓挖矿===>".$val->id);
                            //还有进行收益
                            $wallet = UsersWallet::where('user_id',$val->user_id)->where('currency', $val->to_name)->first();
                            $number = $val->money * $rate* 0.01;
                            // 改
