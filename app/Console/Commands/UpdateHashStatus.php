...
class UpdateHashStatus extends Command
{
    ...
    {   
        ...
        try {
            ...
            $currency_type = $currency->type;//基于哪个区块链
            if (!in_array($currency_type, ["usdt", "btc", "eth", "erc20",'omni','bch'])) {
                throw new \Exception('不支持的币种');
            }
            $currency_type == 'erc20' &&  $currency_type = 'eth';
            $currency_type == 'omni' &&  $currency_type = 'btc';
            ...
            if ($result['code'] == 0) {
                ...
                if ($data->type == 0) {
                    ...
                    //如果是主币还应更新掉主币消耗的手续费
                    if (in_array($currency->type, ['eth', 'btc','bch'])) {
