...
class C2CInfo extends Model
{
    protected $table = 'c2c_info';
    ...
    public function getCurrencyLogoAttribute(){
        return $this->hasOne(Currency::class,'id','currency_id')->value('logo');
    }
    ...
    public function getWayNameAttribute(){
        if ($this->attributes['way'] == 'ali_pay'){
            ...
            $po=UserCashInfo::where('user_id',$this->attributes['seller_id'])->first();
            if ($po&&$po->bank_name){
                return $po->bank_name;
            }else{
                return "";
    ...
    public static function isHasIncompleteness($id){
        $is_deal = C2CUserOrder::where('legal_deal_send_id', $id)->pluck('is_sure')->toArray();
//        var_dump($is_deal);die;
        ...
        try{
            ...
            if (!empty($results)){
                foreach ($results as $result){
                    ...
                    C2CUserOrder::cancelLegalDealById($result->id,AccountLog::C2C_DEAL_AUTO_CANCEL);
    ...
    public function legalDeal(){
        return $this->hasOne(C2CUserOrder::class,'legal_deal_send_id','id');
    }
}
