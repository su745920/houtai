...
class UserChat extends Model
{
    ...
    {
        isset(self::$config['worker_push_url']) || self::$config['worker_push_url'] = config('socketio.worker_push_url') ?? \Request::server('HTTP_HOST');
        isset(self::$config['text_worker_port']) || self::$config['text_worker_port'] = config('socketio.text_worker_port');
    }

    public function getFromAvatarAttribute()
    {
        return $this->hasOne(Users::class,'id','from_user_id')->value('head_portrait');
    }
    ...
    public static function sendChat($data)
    {
        ...
        $worker_push_url = config('socketio.worker_push_url') ?? \Request::server('HTTP_HOST');
        $http_worker_port = config('socketio.http_worker_port');
    ...
    public static function sendText($data)
    {
        ...
        try {
            ...
            // 尝试一次重新连接再发送
            // fclose($client); //事实上断线后是无法再close连接资源的
            try {
