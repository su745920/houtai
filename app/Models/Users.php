...
{
    public static function getByUsername($string)
    {
        if (empty($string)) {
            return "";
        }
    ...
    protected static $roleList = [
        ...
        MicroOrder::RESULT_RISE_PROFIT => '涨赢',
        MicroOrder::RESULT_RISE_LOSS => '跌赢',
    ];
    ...
    {//帐户usdt余额
        ...
        //return isset($wal->change_balance) ? $wal->change_balance : '0.00000';
        $usdt= isset($wal->change_balance) ? $wal->change_balance : '0.00000';
        $usdt2= isset($wal->lever_balance) ? $wal->lever_balance : '0.00000';
     ...
     public function getRealAttribute()
    {
        $value = $this->attributes['id'] ?? 0;
        if ($value) {
            $user_real = DB::table('user_real')->where('user_id', $value)->where('advanced_user',2)->first();
            if($user_real && $this->attributes['is_realname'] == 2) {
                return 1;
            }
        }
        return 0;
    ...
    public function getParentNameAttribute()
    {
        ...
        // }
        try {
            $value = $this->attributes['parent_id'] ?? 0;

            if (!empty($value)) {
                $p = Users::where('id', $value)->first();
    ...
    {
        return $this->hasOne(C2CUserOrder::class, 'seller_id', 'id');
    }
}
