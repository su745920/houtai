...
{
    public static function getMatchId($currency_id)
    {
    ...
    public function getRMatchNameAttribute()
    {
        $clone_name = $this->currency->clone_name ?? '';
        if ($clone_name) {
            $currency_name = $clone_name;
        ...
        return strtolower($currency_name);
    }
    ...
    public static function getHuobiMatchs()
    {
     ...
     public static function getAlltickMatchs()
    {
        $currency_match = self::with(['legal', 'currency'])
            ->whereIn('market_from',[3,4])
            ->where('is_display', 1)
            ->get();
        $currency_match->transform(function ($item, $key) {
            $item->addHidden('currency');
            $item->addHidden('legal');
            $item->append('r_match_name');
            $item->match_name = $item->r_match_name;
            $item->addHidden('r_match_name');
            return $item;
        });
        return $currency_match;
    }
