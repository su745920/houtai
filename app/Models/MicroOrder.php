...
class MicroOrder extends Model
{
    ...
    const RESULT_RISE_PROFIT = 2; //涨盈
    const RESULT_RISE_LOSS = 3; //涨亏
    ...
    protected static $resultList = [
        ...
        self::RESULT_RISE_PROFIT => '涨盈',
        self::RESULT_RISE_LOSS => '跌盈',
    ];
    ...
    {
        $user = $this->user();
        $account = $user->value('email') == "" ? $user->value('phone') :$user->value('email');
    ...
    {
        // try{
            $user_profile = $this->user->userReal;
            if($user_profile){
