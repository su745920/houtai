...
class LeverTransaction extends Model
{
    ...
    public static function tradeHandle($legal_id, $currency_id, $price, $timestamp = null)
    {
        ...
        $levers = LeverTransaction::where("legal", $legal_id)
            ...
            ->groupBy('user_id')
            ->get();
        //echo '风险率订单:' . count($levers) . '个' . PHP_EOL;
        if (count($levers) > 0) {
            foreach ($levers as $trade) {
                //echo '推送风险率订单:' . $trade->id . '秒' . PHP_EOL;
                //推送风险率和订单
    ...
    protected static function handleUserLever($user_id, $legal_id, $currency_id = 0)
    {
        ...
        try {
            ...
            $result = change_wallet_balance(
                ...
                'Warehouse burst treatment' . $legal_name . 'balance(Return of margin:' . $caution_money_total . ',Total settlement profit and loss:' . $profits_total . ')',
                false,
                0,
                $diff == 0 ? 0 : 1, //1代表有差额
                $extra_data,
        ...
        try {
            ...
            $user_profit = LeverTransaction::where('status', self::TRANSACTION)
                ...
                ->where(function ($query) {
                    $query->where('price', '>', 0)->where('origin_price', '>', 0)->where('update_price', '>', 0);
                })
     ...
     * @return bool
     */
    public static function leverClose($lever_transaction, $closed_type = self::CLOSED_BY_USER_SELF,$last_price = null)
    {
        try {
            DB::beginTransaction();
            ...
            if (
                bc_comp_zero($lever_transaction->origin_price) <= 0
                || bc_comp_zero($lever_transaction->price) <= 0
                || bc_comp_zero($lever_transaction->update_price) <= 0
                ) {
                throw new \Exception('交易行情价格异常');
            }
            if(!$last_price) {
            ...
            // $filename = date('Ymd') . '.log';
            // file_exists($path) || @mkdir($path);
            // error_log(date('Y-m-d H:i:s') . '平仓1'. PHP_EOL, 3, $path . $filename);
            ...
            $price = $lever_transaction->origin_price;
            $diff = $lever_transaction->type == self::BUY ? bc_sub($update_price, $price) : bc_sub($price, $update_price);
            // $profit = bc_mul($diff, $lever_transaction->number);
            // $profit= bc_mul($profit,$lever_transaction->multiple);//乘以杠杆倍数
            
            // 盈亏 =（当前价-开仓价）×手数×杠杆
            $profit= bc_mul($diff,$lever_transaction->share);//乘以手数
            $profit= bc_mul($profit,$lever_transaction->multiple);//乘以杠杆倍数
            ...
            */
            ...
            // error_log(date('Y-m-d H:i:s') . '平仓2'. PHP_EOL, 3, $path . $filename);
            if ($result !== true) {
                throw new \Exception($result);
            }

            // error_log(date('Y-m-d H:i:s') . '平仓3'. PHP_EOL, 3, $path . $filename);
            $lever_transaction->refresh();
            ...
            // error_log(date('Y-m-d H:i:s') . '平仓4'. PHP_EOL, 3, $path . $filename);
            DB::commit();

            // error_log(date('Y-m-d H:i:s') . '平仓4b'. PHP_EOL, 3, $path . $filename);

            //$lever_trades = collect([$lever_transaction]);
            //event(new LeverTradeClosedEvent($lever_trades));

            // error_log(date('Y-m-d H:i:s') . '平仓5'. PHP_EOL, 3, $path . $filename);
