...
class MarketHour extends Model
{
     ...
     */
    public static function getEsearchClient($index = null)
    {
        if (is_null(self::$esClient)) {
         ...
         Log::info("setEsearchMarket0000000000000");
        is_null($index) || self::$esIndex = $index;
        is_null(self::$esIndex) && self::$esIndex = config('elasticsearch.index');
    ...
    public static function setEsearchMarket($market_data)
    {
        ...
        Log::info("setEsearchMarket77777777777777");
        return $response;
    }
    ...
    public static function getLastEsearchMarket($base_currency, $quote_currency, $peroid = '1min')
    {
        ...
        $url="https://api.huobi.pro/market/detail/merged?symbol=".$base_currency."usdt";
    ...
    public static function deleteEsearchMarketById($base_currency, $quote_currency, $peroid, $id)
    {
        $es_client = self::getEsearchClient();
        $type = $base_currency . '.' . $quote_currency . '.' . $peroid;
        $params = [
        ...
        $result = $es_client->delete($params);
        return $result;
    }

    public static function deleteEsearchMarketByQuery($base_currency, $quote_currency, $peroid, $from, $to)
    {
        $size = 0;
        $base_currency = strtoupper($base_currency);
        $quote_currency = strtoupper($quote_currency);
        ...
        $result = $es_client->deleteByQuery($params);
        return $result;
    }

    public static function batchEsearchMarket($base_currency, $quote_currency, $price,$vol, $time)
    {
        $types = [
            '1min' => 5,
            '5min' => 6,
        ...
        foreach ($periods as $key => $period) {
            ...
            try {
                ...
                if (isset($result['_source'])) {
                    ...
                    $data['vol'] =  bc_add($data['vol'], $vol, 6);
                    //unset($data['vol'], $data['amount']); //不影响成交数量
                    
                    self::setEsearchMarket($data);
                } else {
                    //拿不到数据,可能是还没有也有可能是程序错误,为了保险建议不处理
                    $last = self::getLastEsearchMarket($base_currency, $quote_currency,$period);
                    ...
                    $return_data[] = $data;
                    Log::info("开始了444444444444444444");
                    self::setEsearchMarket($data);
                    Log::info("开始了555555555555555");
                }
            } catch (\Exception $e) {
                Log::info("报错了".json_encode($e));
                $data = [
                    'id' => $convert_time,
                    'period' => $period,
                    'base-currency' => $base_currency,
