...
class C2CUserOrder extends Model
{
    ...
    protected $table = 'c2c_user_order';
    ...
    {
        return $this->belongsTo(C2CInfo::class, 'legal_deal_send_id', 'id');
    }
    ...
    {
        $legal = C2CInfo::find($this->attributes['legal_deal_send_id']);
        if (!empty($legal)) {
    ...
    {
        return $this->hasOne(C2CInfo::class, 'id', 'legal_deal_send_id')->value('type');
    }
    ...
    {
        return $this->hasOne(C2CInfo::class, 'id', 'legal_deal_send_id')->value('price');
    }

    public function getHesAccountAttribute()
    {
        $legal_send = C2CInfo::find($this->attributes['legal_deal_send_id']);
        if (!empty($legal_send)) {
    ...
    {
        return C2CInfo::find($this->attributes['legal_deal_send_id'])->way_name;
    }
    ...
    public static function cancelLegalDealById($legal_deal_id, $type = AccountLog::C2C_DEAL_AUTO_CANCEL)
    {
        //DB::beginTransaction();
        try {
            $legal_deal = C2CUserOrder::lockForUpdate()
                ->find($legal_deal_id);
            $legal_deal_send = C2CInfo::lockForUpdate()
            ...
            if ($legal_deal_send->type == 'buy') {
                ...
                $result = change_wallet_balance($users_wallet, 1, -$legal_deal->number, $type, '取消出售给卖家法币,锁定余额减少', 'Cancel the sale of legal currency to the seller,Lock in balance decrease',true);
                if ($result !== true) {
                    throw new \Exception($result);
                }
                $result = change_wallet_balance($users_wallet, 1, $legal_deal->number, $type, '取消出售给卖家法币,余额增加','Cancel the sale of legal currency to the seller,Increase in balance');
                if ($result !== true) {
                    throw new \Exception($result);
                }
                //撤回手续费
                if (bc_comp($legal_deal->out_fee, '0') > 0) {
                    $result = change_wallet_balance($users_wallet, 1, $legal_deal->out_fee, AccountLog::C2C_CANCEL_TRADE_FEE, '取消出售给卖家法币,退回手续费','Cancel the sale of legal currency to the seller,Refund fee');
                    if ($result !== true) {
                        throw new \Exception($result);
                    }
                }
