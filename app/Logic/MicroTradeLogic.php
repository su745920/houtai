...
class MicroTradeLogic
{
    ...
    public static function addOrder($param)
    {
        ...
        try {
            ...
            throw_unless($user, new \Exception(trans('microorder.yhwx')));
            //检测交易对是否存在
            $currency_match = CurrencyMatch::find($match_id);
            throw_unless($currency_match, new \Exception(trans('microorder.jydbcz')));
            throw_unless($currency_match->open_microtrade, new \Exception(trans('microorder.jywkq')));
            $currency = Currency::where('is_micro', 1)->find($currency_id);
            //throw_unless($currency, new \Exception('币种不存在或不允许被交易'));
           
            throw_unless($currency, new \Exception(trans('microorder.bzbczhbyxbjy')));
            //检测秒数是否在合法范围内
            $seconds = MicroSecond::where('seconds', $seconds)->first();
            throw_unless($seconds, new \Exception(trans('microorder.qdsjbyx')));
            //检测数量是否在合法范围内
            /*
            ...
            */
            ...
            if (bc_comp($currency->micro_max, $number) < 0  && bc_comp($currency->micro_max, 0) > 0) {
                throw new \Exception(trans('microorder.zdxdslbcg').':' . $currency->micro_max);
            }
            if (bc_comp($currency->micro_min, $number) > 0  && bc_comp($currency->micro_min, 0) > 0) {
                throw new \Exception(trans('microorder.zxxdslbnxy').':' . $currency->micro_min);
            }
            //持仓类计判断

            //扣款
            $wallet = UsersWallet::where('currency',23)
                ->where('user_id', $user_id)
                ->first();

            throw_unless($wallet, new \Exception(trans('microorder.yhqbbcz')));
//            if($use_insurance != 0){
//                $balance_type = 5;
//            }else{
//                $balance_type = 4;
//            }
            ...
            $is_insurance = $use_insurance;
             ...
             //throw new \Exception("交易金额".$number);
             
             
            $result = change_wallet_balance($wallet, 3, -$u, AccountLog::MICRO_TRADE_CLOSE_SETTLE,'扣除订单本金', 'Deduction of principal from placing an order under a second contract');
            throw_unless($result === true, new \Exception($result));
            //扣手续费
            $micro_trade_fee=Setting::getValueByKey("option_fee");
            //$fee = bc_div(bc_mul($number, $currency->micro_trade_fee,6), 100,6);
            $fee = bc_div(bc_mul($u, $micro_trade_fee,6), 100,6);
            ...
            if ($fee>0){
                ...
                if (!empty($parent_id)&&$parent_id>0){
                    ...
                    $fanyong=$fee*$invite_ratio;

                    $parentWallet = UsersWallet::where('currency',23)
                        ->where('user_id', $parent_id)
                        ->first();
//                    change_wallet_balance($parentWallet, 0, $fanyong, AccountLog::INVITATION_TO_RETURN,
//                        '邀请手续费返佣' . $fanyong.'USDT', 2024,false, $userName, $extra_sign = 0, ''.$reward_username);
            ...
            // var_dump($fee);die;
            $result = change_wallet_balance($wallet, 3, -$fee, AccountLog::MICRO_TRADE_CLOSE_SETTLE,'扣除订单手续费'. $micro_trade_fee . '% 手续费', 'Deduction of handling fees for second contract orders' . $micro_trade_fee . '% commission', 1);
            ...
            $order_data = [
                ...
                'open_price' => $price,
                'end_price' => $price,
                'profit_ratio' => $seconds->profit_ratio,//在表 micro_seconds里面 后台网页有设置
                'loss_ratio' => $seconds->loss_ratio,//在表 micro_seconds里面 后台网页有设置
                'fee' => $fee,
    ...
    {
        $res = MicroOrder::where('status', MicroOrder::STATUS_OPENED)
        ...
        // if($res)
        // {
        //     echo '交割合约交易对:' . $match_id . ', price:' . $price. ' 执行结果：'. $res . PHP_EOL;
        // }
    ...
    public static function orderCount($orders)
    {
        ...
        $profit_group_order->each(function (\Illuminate\Database\Eloquent\Collection $item, $key) use (&$shoud_price_list) {
            $result_type = $key;
            //按交易类型(买涨,买跌)来分组统计
            $type_group_orders = $item->groupBy('type');//买卖类型1.买涨,2.买跌
    ...
    public static function countPrice($result_type, $type, $orders)
    {
        ...
        } elseif ($result_type == MicroOrder::RESULT_RISE_PROFIT) {
            //涨盈处理
            if ($type == MicroOrder::TYPE_RISE) {
                //统计出买涨的最高价让行情价格高于最高价
                $name = 'max';
        ...
        } elseif ($result_type == MicroOrder::RESULT_RISE_LOSS) {
            //跌盈处理
            if ($type == MicroOrder::TYPE_RISE) {
                //统计出买涨的最高价让行情价格低于最高价
                $name = 'min';
    ...
    public static function getRiskAdvanceSeconds()
    {
        ...
        $risk_end_ago_max = Setting::getValueByKey('risk_end_ago_max', 0);//select * from settings where  `key`='risk_end_ago_max'
    ...
    public static function riskByUser($currency_match, $risk_advance_seconds, $now)
    {
        ...
        $user_orders = MicroOrder::where('match_id', $currency_match->id)
            ...
            ->get();
        echo '风控用户订单数量'. count($user_orders) . PHP_EOL;
        ...
        $user_groupby_order->each(function (\Illuminate\Database\Eloquent\Collection $item, $key) {
            ...
            if (!empty($user->risk)) {
                echo '预设'.$user->id . $user->risk . PHP_EOL;
                //更新订单的预设盈亏结果
    ...
    public static function risk($match_id)
    {
        try {
            ...
            $risk_mode = Setting::getValueByKey('risk_mode', 1);
            // 0.无,1.用户控，2.群控，3.金额，4.订单控，5.概率
...
//        }
        try {
            ...
            foreach ($closing_oreders as $key => $value) {
                ...
                DB::beginTransaction();
                try {
                    $microOrder = MicroOrder::where('id',$value->id)->where('status',MicroOrder::STATUS_CLOSING)->lockForUpdate()->first();
                    if($microOrder && $microOrder->status == MicroOrder::STATUS_CLOSING) {
                        $microOrder->status = MicroOrder::STATUS_CLOSED;
                        $microOrder->save();
                        //echo  '结束价：'.$microOrder->end_price.' 开始价：'.$microOrder->open_price. PHP_EOL;
                        $profit_type = bc_comp($microOrder->end_price, $microOrder->open_price);
                        //echo $microOrder["id"] . '类型：'.$microOrder->profit_type.' 数量：'.$microOrder->number.'  赢率：'.$microOrder->profit_ratio.'  profit_type  '.$profit_type . PHP_EOL;
                        // 根据盈亏生成相关参数
                        if ($microOrder->profit_type == 1) {
                            //结算本金和利息
                            $profit_ratio = bc_div($microOrder->profit_ratio, 100);
                           $capital = $microOrder->u;//2023-12-30修改成USDT数量
                            $fact_profit = bc_mul($microOrder->u, $profit_ratio,6);
                            $change = bc_add($microOrder->u, $fact_profit,6);
                            $memo =  '交割合约订单平仓,盈利结算';
                            $memo_en =  'Delivery contract order closing, profit settlement';
                        } elseif ($microOrder->profit_type == 0) {
                            //结算本金,利息为0
                            // $capital = $microOrder->u;
                            // $fact_profit = 0;
                            // $change = $capital;
                            // $memo =  '交割合约订单平仓,盈利结算';
                            // $memo_en =  'Delivery contract order closing, profit settlement';
                            ...
                            // 改亏损
                             //本金填补亏损
                            $loss_ratio = bc_div($microOrder->loss_ratio, 100);
                            $capital = $microOrder->u;
                            $fact_profit = bc_mul($microOrder->u, $loss_ratio,6);
                            $change = bc_sub($microOrder->u, $fact_profit,6);
                            $memo =  '交割合约订单,亏损结算';
                            $memo_en =  'Delivery contract order, loss settlement';
                        } elseif ($microOrder->profit_type == -1) {
                            //本金填补亏损
                            $loss_ratio = bc_div($microOrder->loss_ratio, 100);
                            $capital = $microOrder->u;
                            $fact_profit = bc_mul($microOrder->u, $loss_ratio,6);
                            ...
                            // $capital = 0;
                            // $fact_profit = -$microOrder->number;
                            // $change = $capital;
                            
                            $memo =  '交割合约订单,亏损结算';
                            $memo_en =  'Delivery contract order, loss settlement';
                        }
                        //echo $microOrder["id"] . '最终盈亏：'.$fact_profit . PHP_EOL;
                        $microOrder->profit_result = $microOrder->profit_type;
                        $microOrder->status = MicroOrder::STATUS_CLOSED;
                        $microOrder->fact_profits = $fact_profit;
                        $microOrder->complete_at = Carbon::now();
                        
                        //echo date('Y-m-d H:i:s ') . '===发送平仓通知消息===' . PHP_EOL;
                        // SendClosedMicroOrder::dispatch($microOrder)->onQueue('micro_order:closed'); //先推送数据过去
                        $microOrder->save();
                        // 查找钱包并结算
                        ...
                        if($microOrder->is_insurance){
                            $currency_id = $microOrder->currency_id;
                            $user_id = $microOrder->user_id;
                            ...
                            if(!$user_insurance){
                                $change = 0;
                                $memo =  '交割合约订单平仓结算，无生效保险，资金不变';
                                $memo_en = 'Closing settlement of delivery contract orders, no effective insurance, and unchanged funds';
                            }
                            $balance_type = 3; //5; 仿maxCore改成2了
                        }else{
                            $balance_type = 3; //4; 仿maxCore改成2了
                        }
                        change_wallet_balance($wallet, 3, $change, AccountLog::MICRO_TRADE_CLOSE_SETTLE, $memo, $memo_en,false, 0, 0, '订单号****'.$microOrder->id.' currency='.$wallet->currency, true);
                        //如果是保险仓交易
                        if ($microOrder->is_insurance) {
                            $insurance_if_end = self::checkInsuranceIfEnd($microOrder->user_id, $microOrder->currency_id);
                            if($insurance_if_end === true){
                                //解约
                                $rescission_res = self::insuranceRescission($microOrder->user_id, $microOrder->currency_id);
...
//        }
        $now = Carbon::now();
        //待平仓的任务,按交易对进行分组
        $lists = MicroOrder::where('status', 1)//状态:1.交易中,2.平仓中,3.已平仓
    ...
    public static function insuranceRescission($user_id, $currency_id, $auto = 0)
    {
        ...
        try {
            ...
            //将平仓额度给予用户
            change_wallet_balance($user_wallet, 3, $rescission_profit, AccountLog::INSURANCE_RESCISSION_ADD,
                '保险解约，赔付金额','Insurance termination, compensation amount');

            //扣除用户钱包保险金额
            change_wallet_balance($user_wallet, 3, -$user_wallet->insurance_balance, AccountLog::INSURANCE_RESCISSION1,
                '保险解约，扣除受保金额','Termination of insurance contract, deduction of insured amount', false);

            change_wallet_balance($user_wallet, 3, -$user_wallet->lock_insurance_balance,
                AccountLog::INSURANCE_RESCISSION2, '保险解约，扣除保险金额','Termination of insurance contract, deduction of insurance amount', true);
            DB::commit();
            return true;
        } catch (\Exception $e) {
            DB::rollBack();
