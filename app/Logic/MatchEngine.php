        ...
        do {
		    ...
		    $this->info("写完成记录");
                    ...
                    TransactionComplete::create([
                        'user_id' => $in->user_id,
                        'from_user_id' => $out->user_id,
                        'price' => $price,
                        'number' => $number,
                        'currency' => $this->currencyMatch->id,
                        'in_fee' => $in_fee,
                        // 'out_fee' => $out_fee,
                    ]);
                    $this->info("数据入市场");
...
//                    $this->info("数量:{$number}");
	...
	$out = null;
        $in = TransactionIn::where('legal', $this->currencyMatch->legal()->value('id'))
            ->where('currency', $this->currencyMatch->currency()->value('id'))->where('user_id','!=',90)
            ->orderBy('price','DESC')->orderBy('create_time')
            ->first();
	$in = TransactionIn::lockForUpdate()->find($in->id ?? 0);
	if($in){
        $out = TransactionOut::where('legal', $this->currencyMatch->legal()->value('id'))
            ->where('user_id','!=',$in->user_id)->where('currency', $this->currencyMatch->currency()->value('id'))
	    ->where('price', '<=', $in->price ?? 0)->orderBy('price')->first();
        $out = TransactionOut::lockForUpdate()->find($out->id ?? 0);
        }
        if (!$out) {
            $out = TransactionOut::where('legal', $this->currencyMatch->legal()->value('id'))
                ->where('currency', $this->currencyMatch->currency()->value('id'))->where('user_id','!=',90)
                ->orderBy('price')->orderBy('create_time')
                ->first();
            $out = TransactionOut::lockForUpdate()->find($out->id ?? 0);
	    if($out){
            $in = TransactionIn::where('legal', $this->currencyMatch->legal()->value('id'))
                ->where('currency', $this->currencyMatch->currency()->value('id'))
                ->where('price', '>=', $out->price ?? 0)->orderBy('price','DESC')->first();
	    $in = TransactionIn::lockForUpdate()->find($in->id ?? 0);
	    }
        }

        if (!$out || !$in) {
            $this->info("{$this->currencyMatch->symbol}没有匹配单");
            sleep(1);
            return false;
        }
        $this->info("{$this->currencyMatch->symbol}开始匹配交易");
        return [
            'in' => $in,
            'out' => $out
        ];
    ...
    public function changeBalance($in, $out, $base_balance, $quote_balance, $dec_fee_base_balance, $dec_fee_quote_balance)
    {
        ...
        change_wallet_balance($inUserCurrencyWallet, 1, $dec_fee_base_balance,
            AccountLog::TRANSACTIONIN_REDUCE_ADD,'撮合交易成交增加','Increase in matching transactions');
        change_wallet_balance($inUserLegalWallet, 1, -$quote_balance,
            AccountLog::TRANSACTIONIN_REDUCE_ADD,'撮合交易成交扣除冻结','Matching transaction deduction freeze', true);
        ...
        change_wallet_balance($outUserLegalWallet, 1, $dec_fee_quote_balance,
            AccountLog::TRANSACTIONIN_SELLER_ADD,'撮合交易成交增加','Increase in matching transactions');
        change_wallet_balance($outUserCurrencyWallet, 1, -$base_balance,
            AccountLog::TRANSACTIONIN_REDUCE_ADD,'撮合交易成交扣除冻结','Matching transaction deduction freeze', true);
    }
    ...
    public function sendES($number, $price)
    {
        $kline_data = MarketHour::batchEsearchMarket($this->currencyMatch->currency_name, $this->currencyMatch->legal_name,
            $price, sctonum($number), now()->timestamp);
