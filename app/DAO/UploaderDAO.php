...
class UploaderDAO
{
    ...
    public static function fileUpload($file, $scene = '')
    {
        ...
        $upload_file_ext_list == '' && $upload_file_ext_list = 'png,jpg,jpeg';
        //文件大小判断
        $upload_file_size *= 51200; //文件上传最大
        $filesize = $file->getSize();
        $filename = $file->getFilename();
        $origin = $file->getClientOriginalName();
        $ext = $file->guessExtension();
        $client_ext = $file->guessClientExtension();
        empty($scene) || $scene .= '/';
        ...
        if (!in_array($ext, $upload_file_ext_list) || $ext == 'php' || $client_ext == 'php') {
            return [
                'state' => $ext . "|" . $client_ext . self::getStateInfo('ERROR_TYPE_NOT_ALLOWED'),
                'url' => '',
                'title' => $filename,
                'original' => $origin,
                'type' => '.' . $ext,
        ...
        // 图片类型检测和处理(过滤掉植入代码的图片)
        in_array($ext, ImageFilterDAO::getImageTypesList()) && ImageFilterDAO::handle($file->getRealPath(), $ext);
        //文件大小校验
