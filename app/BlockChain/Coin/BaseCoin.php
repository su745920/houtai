...
abstract class BaseCoin implements CoinInterface
{
     ...
     * @param string $memo 链上转账备注
     * @return array
     */
    public function transfer($scene, $amount, $receiver, $sender, $sender_private_key, $fee = 0, $captcha = '', $memo = '')
    {
    ...
    //充币监听 插入hash记录
    public function insertChargeHash($data)
    {
        //处理数据
        $da = $this->parseBlockData($data);

        ChargeHash::unguarded(function () use ($da) {
    ...
    {
        if (in_array($data['coin'], ['ETH', 'ERC20'])) {
