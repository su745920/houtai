...
use App\Models\{Agent, AgentMoneylog, Currency, LeverTransaction, TransactionComplete, TransactionOrder, Users,UsersWallet,CurrencyMatch,MicroOrder};
...
class OrderController extends Controller
{
    ...
    public function transactionList(Request $request)
    {
        ...
        // 改
        $node_users = Users::where('agent_note_id',$agent_id)->pluck('id')->all();
     ...
     // 秒合约订单
    public function micro_order_list(Request $request)
    {
        ...
        $results = MicroOrder::with(['currency', 'currencyMatch', 'user'])
            ->where(function ($query) use ($now_agent_id) {
                ...
                // 改
                 $query->whereHas('user', function ($query) use ($now_agent_id) {
                    $now_agent_id > 0 && $query->where('agent_note_id',$now_agent_id);
                });
            })
    ...
    public function sel_agent_arr($arr = array())
    {
     ...
     // 我的秒合约订
    public function userMicroIndex(Request $request)
    {
        $id = $request->input('id', null);
        $currencies = Currency::where('is_micro', 1)->get();
        $currency_matches = CurrencyMatch::where('open_microtrade', 1)->get();
        $authorityList=session()->get("authorityList");
        return view('agent.user.microlist',['user_id' => $id])
            ->with('authorityList',$authorityList)
            ->with('currencies', $currencies)
            ->with('currency_matches', $currency_matches);
    }
    ...
    // 我的秒合约订单
    public function userMicroList(Request $request)
    {
        $currency_id = $request->input('currency_id', -1);
        $user_id = $request->input('user_id', 0);
        $match_id = $request->input('match_id', -1);
