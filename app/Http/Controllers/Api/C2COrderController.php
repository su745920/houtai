...
class C2COrderController extends Controller
{
    public function sellCoin(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $legal_deal_send = C2CInfo::lockForUpdate()->find($deal_send_id);
            ...
            $fee = 0;
            if ($legal_deal_send->type == 'sell') { //求购
                ...
                // do something
                if ($users_wallet->lever_balance < $should_deduct_number) {
                    DB::rollback();
...
//            }
    ...
    public function userOrderList(Request $request)
    {
        ...
        $is_sure = $request->input('is_sure', 0);
        ...
        $results = C2CUserOrder::where('user_id', $user_id)->whereHas('legalDealSend');
        if (!empty($type)) {
        ...
        if ($is_sure=="0"){
            $results = $results->whereIn('is_sure', [0,3]);
        }
        if ($is_sure=="1"){
            $results = $results->whereIn('is_sure', [1,2]);
        }
    ...
    /**
     * 购买币种不需要交易密码 去掉 'check_pay_password:c2c'
     * @param Request $request
     * @return \Illuminate\Http\JsonResponse
     */
    public function buyCoin(Request $request)
    {
        ...
        $pay_method=$request->input('pay_method',null);
...
//        }
        ...
        try {
            ...
            $money = bc_mul($legal_deal_send->total_number, $legal_deal_send->price, 6);
            $number = $request->input('amount', null);
            ...
            if ($legal_deal_send->type == 'buy') { //求购
                ...
                $fee = bc_mul($number, $c2c_sell_fee);
                $should_deduct_number = bc_add($number, $fee);
                $total_number=$legal_deal_send->total_number;
                $surplus_number=$legal_deal_send->surplus_number;

                if ($number>=$surplus_number){
                    $legal_deal_send->is_done = 1;
                }else{
                    $legal_deal_send->surplus_number=$legal_deal_send->surplus_number-$number;
                    if ($legal_deal_send->surplus_number < 0){
                        $legal_deal_send->surplus_number=0;
                    }
            ...
            $legal_deal->number = $number; //交易数量
            $legal_deal->create_time = time();
            $legal_deal->pay_method=$pay_method;
            $legal_deal->type=$request->input('type');
            $str = date('Ymd') . str_pad(mt_rand(1, 99999), 5, '0', STR_PAD_LEFT);
    ...
    public function legalDealPlatform(Request $request)
    {
        ...
        $results = C2CInfo::where('currency_id', $currency_id)->where('is_done', 0)->where('type', $type)->orderBy('id', 'desc')->paginate($limit);

        foreach($results as $item){
            $seller_id=$item->seller_id;

            $seller= App\Models\Seller::getById($seller_id);
            if (!empty($seller)){
                $c2c_name=$seller->name;
                $item->c2c_name=$c2c_name;
                $success_order=$seller->success_order;
                if (!empty($success_order)){
                    $item->success_order=$success_order;
                }else{
                    $item->success_order=0;
                }
                $success_rate=$seller->success_rate;
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function savePublishInfo(Request $request)
    {
        $type = $request->input('type', null);
        ...
        try {
            ...
            if ($type == 'sell') {   //如果发布出售信息
                ...
                //扣除法币余额
                $result = change_wallet_balance($wallet, 0, -$total_number, AccountLog::C2C_DEAL_SEND_SELL, '用户发布c2c交易法币出售，扣除法币余额','User releases C2C transaction for sale in legal currency，Deduction of legal currency balance');
                if ($result !== true) {
                    throw new \Exception($result);
                }
                //增加法币冻结余额
                $result = change_wallet_balance($wallet, 0, $total_number, AccountLog::C2C_DEAL_SEND_SELL, '用户发布c2c交易法币出售，扣除法币余额', 'User releases C2C transaction for sale in legal currency，Deduction of legal currency balance',true);
                if ($result !== true) {
                    throw new \Exception($result);
                }
                //扣除手续费
                $result = change_wallet_balance($wallet, 0, -$fee, AccountLog::C2C_TRADE_FEE, '交易扣除手续费','Transaction fee deducted');
                if ($result !== true) {
                    throw new \Exception($result);
                }
            } else {
            ...
            $legal_deal_send = new C2CInfo();
            $legal_deal_send->seller_id = $user_id;
            $legal_deal_send->currency_id = $currency_id;
            if ($type == 'sell') {//以用户为中心 商家卖币 对于用户来说就买币
                $legal_deal_send->type = 'buy';
            }else{
                $legal_deal_send->type = 'sell';
            }

            $alipay=$request->input("alipay","");

            if ($alipay=="1"){
                $legal_deal_send->alipay = 1;
            }else{
                $legal_deal_send->alipay = 0;
            }

            $wechat=$request->input("wechat","");
            if ($wechat=="1"){
                $legal_deal_send->wechat = 1;
            }else{
                $legal_deal_send->wechat = 0;
            }
    ...
    public function sellerInfo(Request $request)
    {
        ...
        $lists = C2CInfo::where('seller_id', $id);
        //是否完成
        if ($was_done == 'true') {
    ...
    public function legalDealSendInfo(Request $request)
    {
        ...
        $legal_deal_send = C2CInfo::find($id);
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function aaaaasellCoin(Request $request)
    {
        $deal_send_id = $request->input('id', null);
        if (empty($deal_send_id)) {
            return $this->error('参数错误');
    ...
    public function sellerLegalDealList(Request $request)
    {
        ...
        $results = C2CUserOrder::where('seller_id', $seller->id);
        if (!empty($type)) {
            $results = $results->whereHas('legalDealSend', function ($query) use ($type) {
                $query->where('type', $type);
            });
        ...
        if ($is_sure=="0"){
            $results = $results->whereIn('is_sure',[0,3])->orderBy('id', 'desc')->paginate($limit);
        }else{
            $results = $results->whereIn('is_sure', [1,2])->orderBy('id', 'desc')->paginate($limit);
        }
    ...
    public function legalDealInfo(Request $request)
    {
        ...
        $legal_deal = C2CUserOrder::find($id);
        if (empty($legal_deal)) {
            return $this->error('无此记录');
        }
    ...
    public function userLegalDealCancel(Request $request)
    {
        ...
        try {
            ...
            C2CUserOrder::cancelLegalDealById($id);
            DB::commit();
    ...
    public function legalDealSellerList(Request $request)
    {
        ...
        // }
        $results = C2CUserOrder::where('legal_deal_send_id', $id)
            ->orderBy('id', 'desc')
            ->paginate($limit);
        return $this->pageDate($results);
    }
...
//            }
            ...
            $legal_send = C2CInfo::find($legal_deal->legal_deal_send_id);
            if (empty($legal_send)) {
                DB::rollback();
                return $this->error('订单异常');
            }
//            if ($legal_send->type == 'sell') {//特别注意 商家发布时 采用相反的方向
//                DB::rollback();
...
//            $seller->seller_balance += $legal_deal->number;
    ...
    public function backSend(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $legal_send = C2CInfo::lockForUpdate()->find($id);
            if (empty($legal_send)) {
                DB::rollback();
                return $this->error('无此记录');
            }
            $is_deal = C2CUserOrder::where('legal_deal_send_id', $id)->where('is_sure', '!=', 2)->first();
            if (!empty($is_deal)) {
                DB::rollback();
            ...
            if ($legal_send->type == 'sell') {
                ...
                //冻结退回余额
                $result = change_wallet_balance($wallet, 1, -$legal_send->total_number, AccountLog::C2C_DEAL_BACK_SEND_SELL, '卖家撤回C2C发布法币出售,冻结资金减少', 'The seller withdraws C2C to issue legal currency for sale,Decrease in frozen funds',true);
                if ($result !== true) {
                    throw new \Exception($result);
                }
                $result = change_wallet_balance($wallet, 1, $legal_send->total_number, AccountLog::C2C_DEAL_BACK_SEND_SELL, '卖家撤回C2C发布法币出售,余额从冻结资金退回','The seller withdraws C2C to issue legal currency for sale,Balance returned from frozen funds');
                if ($result !== true) {
                    throw new \Exception($result);
                }
                //退回手续费
                if (bc_comp($legal_send->out_fee, '0') > 0) {
                    $result = change_wallet_balance($wallet, 1, $legal_send->out_fee, AccountLog::C2C_CANCEL_TRADE_FEE, '卖家撤回C2C发布法币出售,退回手续费','The seller withdraws C2C to issue legal currency for sale,Refund fee');
                    if ($result !== true) {
                        throw new \Exception($result);
                    }
                }
            ...
            $legal_send->delete();
            C2CUserOrder::where('legal_deal_send_id', $id)->delete();
