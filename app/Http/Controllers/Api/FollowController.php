...
{
    public function bigData(){//跟单首页数据统计
        //1.跟单总资产
        $user_id = Users::getUserId();
        $sql="select  SUM(l.caution_money) as allGendanAssets, sum(fact_profits) as allFactProfits  from lever_transaction l WHERE l.user_id=".$user_id." and  trader_uid >0";
        $gendanAssets = DB::select($sql);
        $po = $gendanAssets[0];
        $allGendanAssets=$po->allGendanAssets;
        if (empty($allGendanAssets)){
            $allGendanAssets=0.00;
        }

        $allFactProfits=$po->allFactProfits;
        if (empty($allFactProfits)){
            $allFactProfits=0.00;
        }
        //当前跟单资产
        $sql="select  SUM(l.caution_money) as currentAssets  from lever_transaction l WHERE l.user_id=".$user_id." and  trader_uid >0 and  status =". LeverTransaction::TRANSACTION;
        $currentAssetsList = DB::select($sql);
        $currentAssetsPO = $currentAssetsList[0];
        $currentAssets=$currentAssetsPO->currentAssets;
        if (empty($currentAssets)){
            $currentAssets=0.00;
        }


        $resultVO["allGendanAssets"]=$allGendanAssets;//跟单资产(USDT)
        $resultVO["allFactProfits"]=$allFactProfits;//累计收益
        $resultVO["currentAssets"]=$currentAssets;//累计收益
        return $this->success($resultVO);

    }
   //我选择的交易员 2023-06-27
    public function myTraderPage(){
        $user_id = Users::getUserId();
        $sql="(select  u.head_portrait,u.nickname,f.total_investment_amount,balance,gains_losses,pre_deducted_profit,dividend_settled,f.follow_user_id from users u,follow f where f.status=1 and  u.id=f.follow_user_id and f.user_id=".$user_id.") as cc";
        $list = DB::table(DB::raw($sql))->paginate(10);
        if (!empty($list)){
            foreach ($list as $item){
                $dividend_settled=$item->dividend_settled;
                if (empty($dividend_settled)){
                    $item->dividend_settled = 0;
                }
                $pre_deducted_profit=$item->pre_deducted_profit;
                if (empty($pre_deducted_profit)){
                    $item->pre_deducted_profit = 0;
                }
                $gains_losses=$item->gains_losses;
                if (empty($gains_losses)){
                    $item->gains_losses = 0;
                }
            }
        }
    ...
    public function follow(Request $request)
    {
        ...
        //try {
            $singleOpeningAmount=$request->input("singleOpeningAmount");//单笔开仓金额
            $totalInvestmentAmount=$request->input("totalInvestmentAmount");//投入总金额
            $dailyInvestmentLimit=$request->input("dailyInvestmentLimit");//每日投入上限(选填)
            $leverType=$request->input("leverType");//跟单杠杆 0:跟随交易员  1:自定义

            $user_id = Users::getUserId();
            $trader_user_id = $request->trader_user_id;
            $type = $request->type; //跟随类型：1固定比例跟随 2固定手数跟随
            $number = $totalInvestmentAmount; //跟随数量
            //检查钱包余额
        $wallet = UsersWallet::where("user_id", $user_id)
            ->where("currency", "23")    //USDT
            ->lockForUpdate()
            ->first();

        $walletBalance = $wallet->lever_balance;
        if ($walletBalance <  $totalInvestmentAmount){
            //throw new \Exception(trans('lever.yebz') . $totalInvestmentAmount);
            return $this->success(trans('lever.yebz') . $totalInvestmentAmount);
        }
            ...
            $leverNum=0;
            if (!empty($leverType)&&$leverType==1){
                $leverNum=$request->input("leverNum");
            }
            //
        $result = change_wallet_lock_balance(
            $wallet,
            2, //3, maxCore
            -$totalInvestmentAmount,
            AccountLog::LEVER_TRANSACTION,
            '提交跟单保证金' . $totalInvestmentAmount ,
            12,
            true,
            0,
            0,
        ...
        if ($result !== true) {
            ...
            Follow::updateOrCreate([
                ...
                'type' => $type,
                'status' => 1,
                'single_opening_amount'=>$singleOpeningAmount,//单笔开仓金额
                'total_investment_amount'=>$totalInvestmentAmount,//投入总金额
                'daily_investment_limit'=>$dailyInvestmentLimit,//每日投入上限(选填)
