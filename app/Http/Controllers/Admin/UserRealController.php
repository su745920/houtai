...
class UserRealController extends Controller
{
    public function index()
    {
        ...
        return view("admin.userReal.index",['authorityList' =>$authorityList]);
    }
    ...
    public function real_change(Request $request) {
        $user_id = $request->input('id', 0);
        
        $user = Users::find($user_id);
        
        $real = 0;
        $user_real = UserReal::where('user_id', $user_id)->where('advanced_user',2)->first();
        if($user_real && $user->is_realname == 2) {
            $real = 1;
        }
