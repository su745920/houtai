...
class NewsController extends Controller
{
    ...
    public static function newsCateList($num = 0, $id = 0)
    {
        if($id == 0) {
            if($num != 0) {
                $discuss = NewsCategory::orderBy('sorts', 'asc')->paginate($num);
            } else {
                $discuss = NewsCategory::orderBy('sorts', 'asc')->get();
            }
        } else {
            if($num != 0) {
                $discuss = NewsCategory::where('id', $id)->orderBy('sorts', 'asc')->paginate($num);
            } else {
                $discuss = NewsCategory::where('id', $id)->orderBy('sorts', 'asc')->get();
            }
        }
        return $discuss;
    }
    ...
    */
    ...
    public function add(Request $request)
    {
        ...
        $user_list = Users::select('email','id')->get();
    ...
    */
    ...
    public function postAdd(Request $request)
    {
        ...
        // $news->cover = empty($cover) ? URL("images/zwtp.png"):$cover;
         ...
         $thumbnail0=$request->input("thumbnail0");
        if (!empty($thumbnail0)&&strlen($thumbnail0)>10){
            $news->thumbnail= $thumbnail0;
        }else{
            $news->thumbnail = empty($thumbnail) ? URL("images/zwtp.png"):$thumbnail;
        }

        $cover0=$request->input("cover0");
        if (!empty($cover0)&&strlen($cover0)>10){
            $news->cover = $cover0;
        }else{
            $news->cover = empty($cover) ? URL("images/zwtp.png"):$cover;
        }
        ...
        $news->user_id = $request->input('user_id', null);
        $result = $news->save();
