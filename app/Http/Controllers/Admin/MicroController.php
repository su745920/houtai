...
class MicroController extends Controller
{
    ...
    {
        $authorityList=session()->get("authorityList");
        return view('admin.micro.config',['authorityList' =>$authorityList]);
    
    }
     public function configIndex(Request $request)
    {
        $limit = $request->get('limit', 10);
        $list = MicroSecond::orderBy('id', 'desc')
        ->paginate($limit);
    ...
    public function configEdit(Request $request)
    {
        ...
        $result = MicroSecond::findOrNew($id);

        return view('admin.micro.config_edit', ['result' => $result]);
    }
    ...
    public function configdoEdit(Request $request)
    {
        ...
        $profit_ratio = $request->post("profit_ratio");
        ...
        $res = MicroSecond::find($id);
        if (empty($res)) {
            return $this->error("数据未找到");
        }
        
        $res->seconds = $seconds;
        $res->profit_ratio = $profit_ratio;
        $res->loss_ratio = $loss_ratio;
    ...
    public function microconfigStatus(Request $request)
    {
        $id = $request->input('id', 0);

        $user = MicroSecond::find($id);
        if (empty($user)) {
            return $this->error('参数错误');
        }
        if ($user->status == 1) {
