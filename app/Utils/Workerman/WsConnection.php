...
use App\Models\{CurrencyMatch, CurrencyQuotation, MarketHour,HqPrice};
...
class WsConnection
{
    ...
    public function onConnect($con)
    {
        ...
        if ($this->worker_id < 8) {
            ...
            $this->depthTimer = Timer::add($this->send_freq/3, [$this, 'sendDepthData'], [], true);
        ...
        if ($this->worker_id == 0) {
            ...
            $this->microTradeHandleTimer = Timer::add($this->micro_trade_freq, [self::class, 'handleMicroTrade'], [], true);
            $this->handlePriceTimer = Timer::add(1, [$this, 'sendPriceFloatData'], [], true);
        }
        
        
        // 连接到订阅服务器
        // \Channel\Client::connect(config('socket.channel_ip'), config('socket.channel_port'));
        // \Channel\Client::on("market_from_change", function ($event_data) use ($con) {
        //     $currency_match = $event_data['currency_match'];
        //     if ($event_data['after'] == 2) {
        //         $this->subscribe($currency_match);
        //     } elseif ($event_data['before'] == 2) {
        //         // $this->unsub($currency_match);
        //     }
        // });
    }
    ...
    public function onClose($con)
    {
        ...
        ($this->worker_id == 8 && $this->depthTimer) && Timer::del($this->depthTimer);
        $this->handleTimer && Timer::del($this->handleTimer);
        $this->handlePriceTimer && Timer::del($this->handlePriceTimer);
    ...
    {
        $currency_matches = CurrencyMatch::getHuobiMatchs();
     ...
     * @return void
     */
    public function sendPriceFloatData()
    {
       $list = HqPrice::orderBy('id', 'desc')->limit(1)->get();
       foreach ($list as $k => $v) {
          $data['type'] = 'message';
          $data['symbol'] = $v->symbol;
          $data['float_price'] = $v->float_price;
          $data['currency_id'] = $v->currency_id;
          $data['currency_name'] = $v->currency_name;
          SendMarket::dispatch($data)->onQueue('message');
       }
    }
        ...
        if ($this->worker_id < 8) {
            ...
            foreach ($market_data as $period => $data) {
                foreach ($data as $key => $symbol) {
                    ...
                    //echo json_encode($symbol['kline_data']).PHP_EOL;
                    SendMarket::dispatch($symbol['kline_data'])->onQueue('kline.all');
                    $queue_num = substr("0{$this->worker_id}", -2);
                    // EsearchMarket::dispatch($symbol['market_data'])->onQueue("esearch:market:{$queue_num}");
                    
                    if($period == '1min'){
                         echo '处理' . $key . '.' . $period . '数据' . $symbol['kline_data']['close'].PHP_EOL;
                        CurrencyQuotation::getInstance($symbol['kline_data']['legal_id'], $symbol['kline_data']['currency_id'])
                            ->updateData([
                                'now_price' => $symbol['kline_data']['close'],
                            ]);
