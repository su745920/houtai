...
class WorkerAlltickCallback
{
    ...
    protected $interval = 3; //行情处理时间间隔，单位秒，支持小数
    ...
    public function onWorkerStart($worker)
    {
        ...
        if ($worker->id < 8) {
            ...
            $worker->name = 'alltick.ws:' . 'market.kline.' . $period;
        } else {
            $worker->name = 'alltick.ws:' . 'market.depth.step0';
        }
        $ws_con = new WsAlltickConnection($worker->id);
